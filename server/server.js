...
const resolvers = {
	...
	Mutation:{
		...
		async generateDomains(){
			const domains = [];
			for(let prefix of items.filter(item => item.type === "prefix")) {
				for(let sufix of items.filter(item => item.type==="sufix")) {
					let name=prefix.description+sufix.description;
					let checkout=`https://checkout.hostgator.com.br/?a=add&sld=${name.toLowerCase()}&tld=.com.br`;
		...
		async generateDomain(_,args){
			...
			for(let extension of extensions){
				let checkout=`https://checkout.hostgator.com.br/?a=add&sld=${name.toLowerCase()}&tld=${extension}`;
